#!/usr/bin/env node
/**
 * PCA Precalc Generator for CivisAnalysis2
 *
 * Generates PCA precalculated scatter plot data for all standard periods.
 * Replicates the browser-based calculatePrecalc() logic:
 *   1. Load motions for the period
 *   2. Filter top 513 deputies by vote count
 *   3. Build vote matrix (deputies × rollcalls)
 *   4. SVD via numeric.js (same library as browser) → top-2 components
 *   5. Apply setGovernmentTo3rdQuadrant
 *   6. Compute party alignment
 *   7. Save to data/precalc/
 *
 * Usage: node scripts/generate-pca-precalc.js
 */
'use strict';
...
// Load numeric.js (same SVD library used by the browser frontend)
require(path.join(__dirname, '..', 'javascripts', 'external', 'numeric-1.2.6.js'));

const BASE_DIR = path.join(__dirname, '..');
const DATA_DIR = path.join(BASE_DIR, 'data');
const OUTPUT_DIR = path.join(DATA_DIR, 'precalc', 'pca');
...
const PRESIDENTS = [
    ...
    { name: 'Lula (PT)', period: [new Date(2023, 0, 1), new Date(2027, 0, 1)] },
];
...
// ============================================================
// SVD via numeric.js (identical to browser's calcSVD)
// ============================================================
function computePCA(matrixDepXRollCall) {
    // Same logic as calcSVD() in dimensionality-reduction.js
    ...
    var data_deputies;
    if (transposeToSVD) {
        data_deputies = svdDep.V.map(function (row) { return numeric.mul(row, eigenValues).splice(0, 2); });
    } else {
        data_deputies = svdDep.U.map(function (row) { return numeric.mul(row, eigenValues).splice(0, 2); });
    }
...
// ============================================================
function main() {
    console.log('=== PCA Precalc Generator ===\n');
    ...
    for (var pt = 0; pt < periodTypes.length; pt++) {
        ...
        for (var pidx = 0; pidx < periods.length; pidx++) {
            ...
            // Build dense vote matrix (N x M) — same as browser's createMatrixDeputiesPerRollCall
            // Vote encoding: Sim(0)->1, Não(1)->-1, rest->0
            var matrixDepXRC = numeric.rep([N, M], 0);
            ...
            for (var di = 0; di < depIDs.length; di++) {
                ...
                for (var vi = 0; vi < dv.length; vi++) {
                    var voteVal;
                    if (dv[vi].v === 0) voteVal = 1;       // Sim
                    else if (dv[vi].v === 1) voteVal = -1;  // Não
                    else voteVal = 0;                        // Abstenção etc → 0

                    matrixDepXRC[di][dv[vi].localRC] = voteVal;
                    var entry = { i: di, j: dv[vi].localRC, v: voteVal };
            ...
            // Compute PCA (SVD) — identical to browser's calcSVD
            var positions = computePCA(matrixDepXRC);
            ...
            for (var i = 0; i < N; i++) {
                deputyNodes.push({
                    deputyID: depIDs[i],
                    scatterplot: [Number(positions[i][0].toPrecision(4)), Number(positions[i][1].toPrecision(4))],
            ...
            console.log('  ' + period.name + ': N=' + N + ', M=' + M + ' -> ' + filename);
        }
    }
