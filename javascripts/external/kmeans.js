    ...
    clusters: function () {
        ...
        return centroids.map(function (centroid) {
            return {
                ...
                points: points.filter(function (point) { return point.label() == centroid.label() }).map(function (point) { return { deputyID: point.deputyID(), name: point.name(), district: point.district(), party: point.party(), location: point.location(), selected: point.selected(), hovered: point.hovered() }; }),
            };
        });
    },
...
function Point(deputy) {
    ...
    this.selected = getterSetter(deputy.selected);
    this.hovered = getterSetter(deputy.hovered);
